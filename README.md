...
It queries the [PyPI JSON API](https://warehouse.pypa.io/api-reference/json.html) directly and in parallel via `httpx`/`asyncio` — no `pip index` call, and the dependencies don't need to be installed — and cross-checks every pinned version against the [OSV.dev](https://osv.dev) vulnerability database. It can also export a [CycloneDX SBOM](#sbom-software-bill-of-materials) and is designed to run unattended in [CI pipelines](#ci-integration).
...
The `Note` column only shows exceptional cases (`unpinned`, `unsupported`, `not found`) — for normal dependencies the Patch/Minor/Major columns already say everything there is to say about available updates, so it stays empty.
...
- **Components**: one per pinned dependency, with a [PURL](https://github.com/package-url/purl-spec) (`pkg:pypi/name@version`) and, when available, a license (from the same PyPI response used for the version check)
...
Only dependencies with a pinned version become components — an SBOM describes a concrete build, so unpinned or unresolvable entries are excluded. As with the update/vulnerability checks, coverage depends on `requirements.txt` being fully resolved (see [Direct, transitive, and constrained dependencies](#direct-transitive-and-constrained-dependencies) above).
...
Full example: [`examples/sample-sbom.json`](examples/sample-sbom.json), generated from [`examples/requirements.txt`](examples/requirements.txt) (see [Direct, transitive, and constrained dependencies](#direct-transitive-and-constrained-dependencies) above for how that file was produced).
...
Renders the same table(s) as the terminal output (plus the vulnerability details table with `--list-vulnerabilities`) as a single self-contained HTML file — inline CSS, no external assets, safe to email or drop on a file share for people who don't use a terminal. Open it directly in a browser.

Full example: [`examples/sample-report.html`](examples/sample-report.html) — download it (or clone the repo) and open it locally, GitHub's file viewer shows HTML as source rather than rendering it. It was generated from the same [`examples/requirements.txt`](examples/requirements.txt) as the SBOM example above, via:
...
- The transitive-dependency warning and constraint cross-check are heuristics based on declared PyPI metadata, not a real dependency resolver — they can miss or over-flag edge cases (see [Direct, transitive, and constrained dependencies](#direct-transitive-and-constrained-dependencies)).
